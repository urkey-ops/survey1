...
        stopTypingInstantly: false, // NEW FLAG
        syncIntervalId: null,
    };
...
        if (appState.stopTypingInstantly) { // Check the new flag
            questionElement.textContent = text;
            appState.stopTypingInstantly = false; // Reset the flag
            return;
        }
...
            // If a typing animation is in progress, finish it immediately.
            appState.stopTypingInstantly = true;
        }
    };
...
    // Main title click for admin access
    mainTitle.addEventListener('click', () => {
        clearTimeout(appState.adminTimer);
        appState.adminClickCount++;
        log(`Admin click count: ${appState.adminClickCount}`);
...
        if (confirm("Are you sure you want to clear all locally stored submissions? This action cannot be undone.")) {
            localStorage.removeItem(LOCAL_STORAGE_KEY);
            showTemporaryMessage("Local storage cleared.", "success");
            hideAdminControls();
        }
    });
...
    // Reset on activity
    document.addEventListener('mousemove', debounce(resetInactivityTimer, 500));
    document.addEventListener('keypress', debounce(resetInactivityTimer, 500));
    document.addEventListener('touchmove', debounce(resetInactivityTimer, 500));
...
            overlay.classList.add('hidden');
            resetInactivityTimer();
            showTemporaryMessage("Auto-submit canceled. Continue your survey.", "info");
        }
    });
...
    appState.syncIntervalId = setInterval(syncData, 60000); // Sync every minute
});
